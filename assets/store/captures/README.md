...
- Demo viewport: 530×600 CSS pixels; device scale 1.
- Settings details viewport: 360×1100; device scale 1.6.
- Preview viewport: 700×1000; same scale as Settings details.
- Popup viewport: 336×650; device scale 1.32.
- Demonstration: `../demo.html`, served by a local HTTP server.

## Raw captures

| File | Pixels | Actual captured surface |
| --- | --- | --- |
| `before.png` | 470×239 | Demo `article`, processing disabled |
| `after.png` | 470×239 | Same element after enabling processing |
| `format.png` | 518×251 | Date format and Format pattern wrappers |
| `zone.png` | 518×221 | Time zone and IANA identifier wrappers |
| `preview.png` | 1030×149 | Actual `.options-preview` element |
| `popup-light.png` | 444×521 | Actual `.popup` with Light appearance |
| `popup-dark.png` | 444×521 | Actual `.popup` with Dark appearance |
...
captured product content.

Store upload images are in `../chrome/` at 1280×800. Browser/language footers
and slide numbers are intentionally absent.
