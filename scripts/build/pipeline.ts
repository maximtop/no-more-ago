...
export function parseBuildRequest(mode: string, argv: string[]): BuildRequest {
    ...
    if (browsers.some((browser) => !BROWSERS.includes(browser))) {
        throw new UsageError(`Unknown browser: ${browsers.find((browser) => !BROWSERS.includes(browser))}`);
    }
...
function removeRecoverableCandidate(artifacts: ArtifactServices, candidate: string, error: BuildError): void {
    try {
        ...
        error.keepTask = true; error.message = `${error.message}; candidate cleanup failed at ${candidate}: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`;
    }
}
...
function pairFromCompilation({ outputPath, taskRoot, artifacts }: { outputPath: string; taskRoot: string; artifacts: ArtifactServices }): { directory: string; zipBytes: Buffer } {
    ...
    try {
        ...
        rmSync(generation, { recursive: true, force: true }); throw error;
    }
}
...
async function buildAll({ workspaceRoot, mode, browsers, taskRoot, artifacts, compilerFactory, events }: BuildContext): Promise<Record<string, { directory: string; zipBytes: Buffer }>> {
    ...
    try {
        ...
        const buildError = error as BuildError; if (!buildError.keepTask) {
...
export async function runBuildCommand({ workspaceRoot = process.cwd(), mode, argv = [], compilerFactory = defaultCompilerFactory, artifacts = createArtifactServices(), events = () => undefined, phaseHooks = {} }: BuildOptions): Promise<Promise<void> | void> {
    ...
    if (request.watch) {
        try {
            return startWatch({ workspaceRoot: root, request, taskRoot, compilerFactory, artifacts, events, phaseHooks });
        } catch (error) {
            try {
                artifacts.cleanupTask(taskRoot);
            } catch { /* retain an actionable failure root */ } throw error;
        }
    }
    let keepTask = Boolean(phaseHooks.keepTask);
    try {
        await buildAll({ workspaceRoot: root, mode: request.mode, browsers: request.browsers, taskRoot, artifacts, compilerFactory, events });
    } catch (error) {
        keepTask ||= Boolean((error as BuildError)?.keepTask); throw error;
    } finally {
        if (!keepTask) {
            try {
                artifacts.cleanupTask(taskRoot);
            } catch { /* preserve actionable publication backups */ }
        }
    }
}
...
function startWatch({ workspaceRoot, request, taskRoot, compilerFactory, artifacts, events, phaseHooks }: { workspaceRoot: string; request: BuildRequest; taskRoot: string; compilerFactory: CompilerFactory; artifacts: ArtifactServices; events: EventSink; phaseHooks: PhaseHooks }): Promise<void> {
    ...
    const finish = () => {
        ...
        const complete = () => {
            if (completed) {
                return;
            } completed = true; if (!preserveTask) {
                try {
                    artifacts.cleanupTask(taskRoot);
                } catch { /* preserve recovery paths */ }
            } phaseHooks.afterCleanup?.(); settled();
        };
        const timeout = setTimeout(complete, 5000);
        if (typeof compiler?.close === "function") {
            compiler.close(() => {
                clearTimeout(timeout); complete();
            });
        } else {
            clearTimeout(timeout); complete();
        }
    ...
    compiler.watch({}, (error, stats) => {
        ...
        if (error || stats?.hasErrors()) {
            emitAfterWatchRearm({ type: "build", sequence: ++sequence, status: "failed", error: error?.message ?? "Compilation failed" }); return;
        }
        ...
        try {
            ...
            try {
                phaseHooks.beforePublish?.(); artifacts.publishModeRoot({ candidate, modeRoot, taskRoot }); phaseHooks.afterPublish?.();
            } catch (error) {
                const buildError = error as BuildError; if (!buildError.keepTask) {
                    removeRecoverableCandidate(artifacts, candidate, buildError);
                } throw buildError;
            ...
            if (pair?.directory) {
                try {
                    rmSync(pair.directory, { recursive: true, force: true });
                } catch { /* preserve only unrecoverable publication state */ }
            }
            if (generationActive) {
                events({ type: "allocation", resource: "generation", action: "end", browser });
            }
