...
function assertSafe(root: string, candidate: string, { allowMissing = false }: { allowMissing?: boolean } = {}): string {
    ...
    if (!inside(absoluteRoot, absolute)) {
        throw new Error(`Path escapes guarded root: ${absolute}`);
    }
    ...
    if (existsSync(absolute)) {
        ...
        if (!inside(canonicalRoot, canonical)) {
            throw new Error(`Real path escapes guarded root: ${absolute}`);
        }
    }
    return absolute;
}
...
function copyTree(source: string, destination: string): void {
    ...
    for (const name of readdirSync(source)) {
        ...
        if (stat.isSymbolicLink()) {
            throw new Error(`Symlink is not allowed: ${from}`);
        }
        if (stat.isDirectory()) {
            copyTree(from, to);
        } else if (stat.isFile()) {
...
function validatePng(file: string, expectedSize: number): void {
    ...
    if (!sawHeader || !sawData || !sawEnd) {
        throw new Error(`PNG is incomplete: ${file}`);
    }
    let decoded;
    try {
        decoded = Buffer.from(unzlibSync(new Uint8Array(Buffer.concat(imageData))));
    } catch (error) {
        throw new Error(`Invalid PNG image data: ${file}`, { cause: error });
    }
    const rowBytes = expectedSize * 4;
    if (decoded.length !== expectedSize * (rowBytes + 1)) {
        throw new Error(`Invalid PNG scanline data: ${file}`);
    }
    for (let row = 0; row < expectedSize; row += 1) {
        if ((decoded[row * (rowBytes + 1)] ?? 5) > 4) {
            throw new Error(`Invalid PNG filter byte: ${file}`);
        }
    }
}
...
function validatePair(directory: string, zipBytes: Buffer, decoder: (data: Uint8Array) => Record<string, Uint8Array> = fflateUnzipSync): { files: string[]; manifest: ArtifactManifest; zipBytes: Buffer } {
    ...
    for (const [label, document] of documents) {
        ...
        if (typeof document !== "string" || !document || document.startsWith("/")) {
        ...
        for (const reference of [...scriptReferences, ...styleReferences]) {
            ...
            if (!files.includes(reference)) {
                throw new Error(`${label} references missing file: ${reference}`);
            }
        }
...
function zipEntries(root: string): Record<string, Uint8Array> {
    ...
    for (const name of listFiles(root)) {
        entries[name] = new Uint8Array(readFileSync(path.join(root, ...name.split("/"))));
    }
    return entries;
}
