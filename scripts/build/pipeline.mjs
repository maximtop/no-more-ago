...
import { BROWSERS, MODES, createRspackConfig } from "../../rspack.config.mjs";
import { assertSafe, createArtifactServices } from "./artifacts.mjs";

export const USAGE = "Usage: pnpm dev|release [chrome|firefox|edge] [--watch]";

export function parseBuildRequest(mode, argv) {
  ...
  const browsers = args.length === 0 ? [...BROWSERS] : [args[0]];
...
function removeRecoverableCandidate(artifacts, candidate, error) {
  try { artifacts.cleanupCandidate(candidate); }
  catch (cleanupError) { error.keepTask = true; error.message = `${error.message}; candidate cleanup failed at ${candidate}: ${cleanupError.message}`; }
}

function compileOnce(config, compilerFactory = rspack) {
...
function pairFromCompilation({ outputPath, taskRoot, artifacts }) {
...
async function buildAll({ workspaceRoot, mode, browsers, taskRoot, artifacts, compilerFactory, events }) {
  const pairs = {};
  ...
  const { candidate, modeRoot } = artifacts.buildCandidateModeRoot({ workspaceRoot, mode, pairs, selected });
  try { artifacts.publishModeRoot({ candidate, modeRoot, taskRoot }); }
  catch (error) { if (!error?.keepTask) removeRecoverableCandidate(artifacts, candidate, error); throw error; }
  events?.({ type: "build", sequence: 1, status: "success", mode, browsers });
  return pairs;
}

export async function runBuildCommand({ workspaceRoot = process.cwd(), mode, argv = [], compilerFactory = rspack, artifacts = createArtifactServices(), events = () => undefined, phaseHooks = {} }) {
  ...
  catch (error) { keepTask ||= Boolean(error?.keepTask); throw error; }
  finally { if (!keepTask) { try { artifacts.cleanupTask(taskRoot); } catch { /* preserve actionable publication backups */ } } }
}

function startWatch({ workspaceRoot, request, taskRoot, compilerFactory, artifacts, events, phaseHooks }) {
  const browser = request.browsers[0]; const outputPath = path.join(taskRoot, `compiler-${browser}`); let compiler; let closing = false; let preserveTask = false; let sequence = 0; let settled;
  ...
  const emitAfterWatchRearm = (event) => { setImmediate(() => { if (!closing) events(event); }); };
