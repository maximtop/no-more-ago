...
async function renderPopup(
    ...
    await act(async () => {
        root.render(
            <PopupApp {...initialProps} {...reportingProps} client={new PopupClient(transport)} />,
        );
    });
    return {
        container,
...
describe("PopupApp contract", () => {
    it("renders hostname, global switch, active status, and no counter UI", async () => {
        ...
        try {
            ...
            expect(rendered.container.textContent).not.toMatch(
                /counter|replacement|options|site switch/i,
            );
            expect(
                Array.from(
    ...
    it.each([
        ...
        [
            ...
            "Rules are not available for example.test yet",
        ],
        [
            {
    ...
    it("ignores a lower revision response and shows the typed save failure alert", async () => {
        const transport: PopupTransport = {
            sendMessage: (message) =>
                ...
                "type" in message &&
                message.type === SET_GLOBAL_ENABLED_MESSAGE
                    ? Promise.resolve({
                        ok: false,
                        error: "save-failed",
                        state: { ...active, revision: 1, globalEnabled: true },
                    })
    ...
    it("rereads a committed state after an ambiguous response and gates revisions", async () => {
        ...
        const transport: PopupTransport = {
            sendMessage: (message) => {
                ...
                if (
                    ...
                    typeof message === "object" &&
                    "type" in message &&
                    message.type === GET_POPUP_STATE_MESSAGE
                ) {
        ...
        try {
            ...
            expect(rendered.container.textContent).toContain(
                "The response was interrupted. Current state was reloaded.",
            );
    ...
    it("shows a disabled mixed switch when both the command and reread are lost", async () => {
        const transport: PopupTransport = {
            sendMessage: () => Promise.reject(new Error("transport lost")),
        };
        const rendered = await renderPopup(active, transport);
        try {
            const input = rendered.container.querySelector(
    ...
    it("renders and edits an exact no-adapter host without calling it unsupported", async () => {
        ...
        try {
            ...
            const siteInput = rendered.container.querySelector(
    ...
    it("fails closed when the site command and reread both fail", async () => {
        const transport: PopupTransport = {
            sendMessage: (message) => {
                if (
                    message &&
        ...
        try {
            ...
            const globalInput = rendered.container.querySelector(
                'input[aria-label="Global enabled"]',
            ) as HTMLInputElement;
            ...
            expect(
                rendered.container.querySelector('input[aria-label="Enabled on github.com"]'),
            ).toBeNull();
            expect(rendered.container.textContent).toContain(
                "Could not confirm whether the change was saved",
            );
    ...
    it.each([
        [
            { ...active, globalEnabled: false, status: "runtime-failed", failure: "registration" },
            "Could not process this page",
        ],
        [
            {
        ...
        "gives relevant current-adapter failures precedence over global-off (%s)",
        async (state, wording) => {
            const rendered = await renderPopup(state);
            try {
                expect(rendered.container.textContent).toContain(wording);
    ...
    it.each([
        ...
        [
            { ...active, globalEnabled: false, status: "global-disabled" },
            "Dates are not working correctly",
        ],
        [
            { ...active, siteEnabled: false, status: "site-disabled" },
            "Dates are not working correctly",
        ],
        [
            { ...active, status: "runtime-failed", failure: "document-status" },
            "Dates are not working correctly",
        ],
        [
            { ...active, hostname: "example.test", hasAdapter: false, status: "no-rules" },
        ...
        "opens exactly one site-specific report from eligible %s state",
        async (state, reason) => {
    ...
    it.each([
        ...
        "hides reporting and performs no browser work for unavailable %s context",
        async (state) => {
    ...
    it.each([
        ...
        [
            { url: "chrome-extension://example/options.html", incognito: false },
            "This page cannot be reported",
        ],
        ...
        "shows an actionable error without navigation for unsafe selected tab %s",
        async (tab, wording) => {
    ...
    it("keeps a private site's selected URL within its existing private window", async () => {
        const fixture = createReportingFixture({
            url: "https://github.com/private/repository?view=issue#private",
            incognito: true,
            windowId: 17,
        });
        const rendered = await renderPopup(
        ...
        try {
            ...
            expect(new URL(fixture.opened[0]?.url ?? "").searchParams.get("current_url")).toBe(
                "https://github.com/private/repository?view=issue#private",
            );
        } finally {
            await rendered.unmount();
        }
