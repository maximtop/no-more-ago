...
describe("custom format validation", () => {
    it("accepts Unicode date tokens and quoted literals", () => {
        expect(validateCustomFormatPattern(DEFAULT_CUSTOM_FORMAT_PATTERN)).toEqual({ ok: true, pattern: DEFAULT_CUSTOM_FORMAT_PATTERN });
        expect(validateCustomFormatPattern("EEEE, d MMMM yyyy 'at' HH:mm")).toEqual({ ok: true, pattern: "EEEE, d MMMM yyyy 'at' HH:mm" });
        expect(validateCustomFormatPattern("do MMMM yyyy")).toEqual({ ok: true, pattern: "do MMMM yyyy" });
    ...
    it.each([
        ["", "empty"], ["   ", "empty"], ["'literal'", "missing-date-token"], ["yyyy-MM-dd '", "unclosed-quote"],
        ["YYYY-MM-dd", "legacy-token"], ["YY", "legacy-token"], ["DD/MM", "legacy-token"], ["D", "legacy-token"],
        ["yyyy ff", "invalid-token"], ["yyyy\u0000-MM", "control-character"]
