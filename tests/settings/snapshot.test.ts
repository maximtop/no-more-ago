...
const v5 = (revision = 0, debugEnabled = false) => ({ schemaVersion: 5 as const, revision, globalEnabled: true, sitePreferences: {}, display: display("system"), debugEnabled });

describe("Settings Snapshot V5", () => {
    it("uses one frozen default with debug logging off", () => {
        expect(DEFAULT_SETTINGS_SNAPSHOT).toEqual({ schemaVersion: 5, revision: 0, globalEnabled: true, sitePreferences: {}, display: DEFAULT_DISPLAY_SETTINGS, debugEnabled: false });
    ...
    it("requires exactly six own fields and a boolean debug flag", () => {
        ...
        expect(parseSettingsSnapshot({ schemaVersion: 4, revision: 0, globalEnabled: true, sitePreferences: {}, display: display("system") })).toBeNull();
        expect(parseSettingsSnapshot({ ...v5(), schemaVersion: 6 })).toBeNull();
        expect(parseSettingsSnapshot(Object.assign(Object.create({ debugEnabled: true }), { schemaVersion: 5, revision: 0, globalEnabled: true, sitePreferences: {}, display: display("system") }))).toBeNull();
    });

    it("preserves strict display, hostname, and time-zone validation", () => {
        expect(parseSettingsSnapshot({ ...v5(2, true), display: { formatMode: "custom", pattern: "do MMMM yyyy", timeZone: { mode: "utc" } } })).not.toBeNull();
        expect(parseSettingsSnapshot({ ...v5(), display: { formatMode: "custom", pattern: "YYYY-MM-dd", timeZone: { mode: "utc" } } })).toBeNull();
    ...
    it("constructs V5 while preserving every field", () => {
        expect(createSettingsSnapshot(3, false, { "github.com": false }, display("utc"), true)).toEqual({ schemaVersion: SETTINGS_SCHEMA_VERSION, revision: 3, globalEnabled: false, sitePreferences: { "github.com": false }, display: display("utc"), debugEnabled: true });
        expect(() => createSettingsSnapshot(-1, true)).toThrow(TypeError);
        expect(() => createSettingsSnapshot(0, true, {}, DEFAULT_DISPLAY_SETTINGS, "yes" as never)).toThrow(TypeError);
    });
});
...
describe("strict schema and policy boundaries", () => {
    ...
    it("requires exactly six own top-level fields and rejects inherited substitutions", () => {
        ...
        expect(parseSettingsSnapshot(Object.assign(Object.create({ debugEnabled: true }), { schemaVersion: 5, revision: 0, globalEnabled: true, sitePreferences: {}, display: display("system") }))).toBeNull();
        expect(parseSettingsSnapshot(Object.assign(Object.create({ globalEnabled: true }), { schemaVersion: 5, revision: 0, sitePreferences: {}, display: display("system"), debugEnabled: false }))).toBeNull();
    });

    it.each([
        "example.test", "sub.example.test", "127.0.0.1", "localhost", "[::1]", "xn--bcher-kva.example", "example.test.", "__proto__", "constructor", "tostring"
    ])("accepts canonical exact hostname %s", (hostname) => {
        expect(isCanonicalHostname(hostname)).toBe(true);
        expect(parseSettingsSnapshot({ ...v5(), sitePreferences: { [hostname]: false } })).toMatchObject({ sitePreferences: { [hostname]: false } });
    });

    it.each([
        "EXAMPLE.TEST", "BÜCHER.example", "bücher.example", "example.test..", "example.test:443", "::1", "https://example.test", "example.test/path", "example.test?query", "example.test#hash", "user:pass@example.test", "*.example.test", " example.test", "example.test "
    ])("rejects noncanonical intent hostname %s", (hostname) => {
        expect(isCanonicalHostname(hostname)).toBe(false);
        expect(parseSettingsSnapshot({ ...v5(), sitePreferences: { [hostname]: false } })).toBeNull();
    });

    it("copies own prototype-like map keys without invoking their prototypes", () => {
        const map = Object.fromEntries([["__proto__", false], ["constructor", true], ["tostring", false]]);
    ...
    it("preserves explicit true as an own preference instead of treating it as missing", () => {
        const parsed = parseSettingsSnapshot({ ...v5(), sitePreferences: { "example.test": true } });
    ...
    it("ignores inherited preference entries and copies only own entries", () => {
        const source = Object.assign(Object.create({ "example.test": false }) as Record<string, unknown>, { "other.test": true });
...
describe("display and timezone snapshot validation", () => {
    it.each([
        ...
        { formatMode: "custom", pattern: "EEEE, do MMMM yyyy 'at' HH:mm", timeZone: { mode: "iana", identifier: "Europe/Nicosia" } }
    ...
    it("rejects inherited custom patterns and inherited display discriminants", () => {
        const inheritedPattern = Object.assign(Object.create({ pattern: "yyyy-MM-dd" }) as Record<string, unknown>, { formatMode: "custom", timeZone: { mode: "utc" } });
        expect(parseDisplaySettings(inheritedPattern)).toBeNull();
        const inheritedMode = Object.assign(Object.create({ formatMode: "system" }) as Record<string, unknown>, { timeZone: { mode: "utc" } });
        expect(parseDisplaySettings(inheritedMode)).toBeNull();
    });

    it.each([
        "yyyy-MM-dd", "dd/MM/yyyy HH:mm", "EEEE, d MMMM yyyy", "do MMMM yyyy 'at' HH:mm", "yyyy '' MM", "t", "yyyy-MM-dd XXX"
    ])("accepts bounded Unicode custom pattern %s", (pattern) => {
        expect(parseDisplaySettings({ formatMode: "custom", pattern, timeZone: { mode: "utc" } })).toMatchObject({ formatMode: "custom", pattern });
    });

    it.each([
        "", "   ", "x".repeat(257), "yyyy-MM-dd\u0000", "yyyy-MM-dd '", "yyyy-MM-dd J", "YYYY-MM-dd", "yyyy-DD-dd", "yyyy D", "'literal'", "'-'"
    ])("rejects bounded/legacy/invalid custom pattern %s", (pattern) => {
        expect(parseDisplaySettings({ formatMode: "custom", pattern, timeZone: { mode: "utc" } })).toBeNull();
    });
    ...
    it.each(["", " UTC", "UTC ", "A//B", "/UTC", "UTC/", "A/../B", "A/./B", "A\\B", "A B", "1Europe", "Europe/\u0000City"])("rejects unsafe timezone identifier %s", (identifier) => {
    ...
    it("accepts structurally valid historical zones without checking ICU availability", () => {
        ...
        expect(parseSettingsSnapshot({ ...v5(), display: { formatMode: "system", timeZone: value } })).not.toBeNull();
    });

    it("freezes parsed nested display and timezone values", () => {
        const parsed = parseSettingsSnapshot({ ...v5(), display: { formatMode: "custom", pattern: "yyyy-MM-dd", timeZone: { mode: "iana", identifier: "UTC" } } });
        expect(parsed).not.toBeNull();
