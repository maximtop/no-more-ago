...
describe("background V3 message contracts", () => {
    ...
    it("validates complete plain JSON diagnostic snapshots and finite trusted metadata", () => {
        ...
        const snapshot = {
            entries: [event],
            environment: { extensionVersion: "1.2.3", browserFamily: "firefox" },
        };
        ...
        expect(isDiagnosticsSnapshot({ entries: [], environment: snapshot.environment })).toBe(
            false,
        );
        expect(
            isDiagnosticsSnapshot({
                entries: [{ ...event, url: "https://github.com/private?token=secret" }],
        ...
        expect(
            isDiagnosticsSnapshot({
                entries: [event],
                environment: { ...snapshot.environment, account: "private" },
            }),
        ).toBe(false);
        expect(
            isDiagnosticsSnapshot({ entries: [event], environment: { browserFamily: "safari" } }),
        ).toBe(false);
        expect(
            isDiagnosticsSnapshot({
                entries: [event],
                environment: { browserFamily: "firefox", extensionVersion: "contains/private" },
            }),
        ).toBe(false);
        expect(
            isDiagnosticsSnapshot({
                ...
                environment: Object.assign(
                    Object.create({ extensionVersion: "2.0" }) as Record<string, unknown>,
                    { browserFamily: "firefox" },
                ),
            }),
        ).toBe(false);
        expect(
            isDiagnosticsSnapshot(
                Object.assign(
                    Object.create({ token: "private" }) as Record<string, unknown>,
                    snapshot,
                ),
            ),
        ).toBe(false);
        ...
        expect(
            isGetDiagnosticsSnapshotResponse(
                Object.assign(Object.create({ snapshot }) as Record<string, unknown>, { ok: true }),
            ),
        ).toBe(false);
        for (const error of [
            "disabled",
    ...
    it("accepts only strict preserving-enable clear outcomes", () => {
        ...
        expect(
            isClearDiagnosticsResponse(
                Object.assign(
                    Object.create({ error: "storage-failed" }) as Record<string, unknown>,
                    { ok: false },
                ),
            ),
        ).toBe(false);
    ...
    it("rejects inherited serializers and accessors in diagnostic responses", () => {
        const event = {
            category: "mutation",
            timestamp: 5,
            hostname: "github.com",
        ...
        const inheritedEntries = Object.setPrototypeOf(
            [event],
            arraySerializer,
        ) as (typeof event)[];
        ...
        for (const invalid of [
            inheritedRoot,
            { ...snapshot, environment: inheritedEnvironment },
            { ...snapshot, entries: [inheritedEvent] },
            { ...snapshot, entries: inheritedEntries },
            { ...snapshot, entries: [{ ...event, stack: inheritedStack }] },
        ]) {
        ...
        const accessor = Object.defineProperty({ ...snapshot }, "environment", {
            enumerable: true,
        ...
        expect(
            isGetDiagnosticsSnapshotResponse(
                Object.assign(Object.create(serializer) as Record<string, unknown>, {
                    ok: true,
                    snapshot,
                }),
            ),
    ...
    it("rejects diagnostic journal responses exceeding five megabytes", () => {
        const event = {
            category: "mutation",
            timestamp: 5,
            hostname: "github.com",
        ...
        const snapshot = { entries: [event], environment: { browserFamily: "firefox" } };
        expect(
            new TextEncoder().encode(JSON.stringify({ entries: snapshot.entries })).byteLength,
        ).toBeGreaterThan(DIAGNOSTICS_MAX_BYTES);
    ...
    it("accepts only the exact recovery request and typed reset responses", () => {
        ...
        expect(isResetAllSettingsMessage({ type: RESET_ALL_SETTINGS_MESSAGE, extra: true })).toBe(
            false,
        );
        const request = Object.create({ type: RESET_ALL_SETTINGS_MESSAGE }) as Record<
            string,
            unknown
        >;
        ...
        expect(isResetAllSettingsResponse({ ok: true, acceptedRevision: 4, state: sites })).toBe(
            true,
        );
        expect(isResetAllSettingsResponse({ ok: false, error: "save-failed", state: sites })).toBe(
            true,
        );
        const inheritedSuccess = Object.assign(
            Object.create({ acceptedRevision: 4 }) as Record<string, unknown>,
            { ok: true, state: sites },
        );
        expect(isResetAllSettingsResponse(inheritedSuccess)).toBe(false);
        const inheritedFailure = Object.assign(
            Object.create({ error: "save-failed" }) as Record<string, unknown>,
            { ok: false, state: sites },
        );
    ...
    it("accepts default reset success and truthful failure projections", () => {
        const ready = {
            availability: "ready" as const,
            revision: 8,
            globalEnabled: false,
            sites: [
                { hostname: "github.com", enabled: false, hasAdapter: true },
                { hostname: "managed.test", enabled: true, hasAdapter: false },
            ],
        };
        const defaults = {
            availability: "ready" as const,
        ...
        expect(isResetAllSettingsResponse({ ok: true, acceptedRevision: 0, state: defaults })).toBe(
            true,
        );
        expect(isResetAllSettingsResponse({ ok: false, error: "save-failed", state: ready })).toBe(
            true,
        );
        expect(
            isResetAllSettingsResponse({ ok: false, error: "save-failed", state: unavailable }),
        ).toBe(true);
        expect(
            isResetAllSettingsResponse({ ok: true, acceptedRevision: 0, state: unavailable }),
        ).toBe(false);
        expect(
            isResetAllSettingsResponse({
                ok: false,
        ...
        expect(
            isResetAllSettingsResponse(
                Object.assign(Object.create({ state: ready }) as Record<string, unknown>, {
                    ok: false,
                    error: "save-failed",
                }),
            ),
        ...
        expect(
            isResetAllSettingsResponse(
                Object.assign(Object.create({ acceptedRevision: 0 }) as Record<string, unknown>, {
                    ok: true,
                    state: defaults,
                }),
            ),
    ...
    it("validates ready and unavailable display projections plus strict save responses", () => {
        ...
        expect(
            isSetDisplaySettingsResponse({ ok: false, error: "invalid-time-zone", state: ready }),
        ).toBe(true);
        expect(
            isSetDisplaySettingsResponse({
                ok: false,
                error: "invalid-format",
                state: { availability: "ready", revision: 3, display: custom, debugEnabled: false },
            }),
        ).toBe(true);
        expect(
            isSetDisplaySettingsResponse({
                ...
                acceptedRevision: 4,
                state: ready,
                refreshFailures: [{ hostname: "github.com", tabId: 3, reason: "tab-update" }],
            }),
        ).toBe(true);
        expect(
            isSetDisplaySettingsResponse({
    ...
    it("accepts only exact debug state and toggle envelopes", () => {
        ...
        expect(isSetDebugEnabledMessage({ type: SET_DEBUG_ENABLED_MESSAGE, enabled: true })).toBe(
            true,
        );
        expect(isSetDebugEnabledMessage({ type: SET_DEBUG_ENABLED_MESSAGE, enabled: "yes" })).toBe(
            false,
        );
        ...
        expect(isSetDebugEnabledResponse({ ok: true, acceptedRevision: 4, state: ready })).toBe(
            true,
        );
        expect(
            isSetDebugEnabledResponse({ ok: true, acceptedRevision: 4, state: ready, extra: true }),
        ).toBe(false);
        expect(
            isSetDebugEnabledResponse({
                ok: true,
    ...
    it("validates the Sites request and complete surface models", () => {
        ...
        expect(
            isSitesState({
                ...sites,
                sites: [{ hostname: "github.com", enabled: true, hasAdapter: true, extra: true }],
            }),
        ).toBe(false);
