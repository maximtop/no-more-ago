...
describe("formatDefaultDate", () => {
    ...
    it("uses the selected UTC and IANA zones without changing locale conventions", () => {
        ...
        expect(formatDateWithPresentation(instant, ["en-GB"], { formatMode: "system", timeZone: { mode: "utc" } }).text).toBe(utcExpected);
        expect(formatDateWithPresentation(instant, ["en-US"], { formatMode: "system", timeZone: { mode: "iana", identifier: "America/New_York" } }).text).toBe(nyExpected);
    });
    ...
    it("preserves Unicode hour-cycle preferences for both 24-hour and 12-hour clocks", () => {
        ...
        for (const [locale, hour12] of [["en-US-u-hc-h23", false], ["en-GB-u-hc-h12", true]] as const) {
            ...
            expect(formatDateWithPresentation(instant, [locale], display).text).toBe(formatter.format(instant));
        }
    });
    ...
    it("honors ordered browser locales and falls back past an unsupported valid locale", () => {
        const instant = new Date("2026-08-23T17:15:00.000Z");
        const display = { formatMode: "system" as const, timeZone: { mode: "utc" as const } };
        const localeLists = [["en-GB", "en-US"], ["en-US", "en-GB"], ["zz-ZZ", "en-GB"]] as const;
    ...
    it("skips the nonexistent New York hour across the spring-forward boundary", () => {
        ...
        for (const { instant, hour, minute } of cases) {
            ...
            expect(formatDateWithPresentation(instant, ["en-GB"], display).text).toBe(formatter.format(instant));
    ...
    it("formats custom patterns with locale and one selected zone", () => {
        const before = formatDateWithPresentation(
            new Date("2026-03-08T06:59:00.000Z"), ["en-US"],
            { formatMode: "custom", pattern: "yyyy-MM-dd HH:mm XXX", timeZone: { mode: "iana", identifier: "America/New_York" } }
        );
        const after = formatDateWithPresentation(
            new Date("2026-03-08T07:01:00.000Z"), ["en-US"],
            { formatMode: "custom", pattern: "yyyy-MM-dd HH:mm XXX", timeZone: { mode: "iana", identifier: "America/New_York" } }
        );
    ...
    it("retains the custom pattern when a saved zone is unavailable", () => {
        expect(formatDateWithPresentation(
            new Date("2026-01-15T12:15:00.000Z"), ["de-DE"],
            { formatMode: "custom", pattern: "d MMMM yyyy", timeZone: { mode: "iana", identifier: "Gone/Zone" } },
            () => false
    ...
    it("contains formatter failures and produces no displayable text", () => {
        expect(formatDateWithPresentation(
            new Date("2026-01-15T12:15:00.000Z"), ["en-US"],
            { formatMode: "custom", pattern: "yyyy ff", timeZone: { mode: "system" } }
