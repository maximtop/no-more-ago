...
const customDisplay = { formatMode: "custom" as const, pattern: "yyyy-MM-dd", timeZone: { mode: "utc" as const } };
...
describe("document status message", () => {
  it("accepts only the exact request and guarded response phases", () => {
    ...
    for (const value of [null, [], {}, { type: DOCUMENT_STATUS_MESSAGE, extra: true }, { type: "other" }]) {
    ...
    expect(isDocumentStatusResponse({ type: DOCUMENT_STATUS_MESSAGE, phase: "unknown" })).toBe(false);
    expect(isDocumentStatusResponse({ type: DOCUMENT_STATUS_MESSAGE, phase: "active", extra: true })).toBe(false);
  });
});
...
describe("revisioned presentation messages", () => {
  it("accepts complete V3 updates and rejects unsafe or structurally incomplete requests", () => {
    expect(isPresentationUpdateMessage({ type: UPDATE_PRESENTATION_MESSAGE, revision: 4, display })).toBe(true);
    expect(isPresentationUpdateMessage({ type: UPDATE_PRESENTATION_MESSAGE, revision: 0, display: { formatMode: "system", timeZone: { mode: "iana", identifier: "America/New_York" } } })).toBe(true);
    expect(isPresentationUpdateMessage({ type: UPDATE_PRESENTATION_MESSAGE, revision: 0, display: customDisplay })).toBe(true);
    for (const value of [
      undefined,
      null,
      ...
      { type: UPDATE_PRESENTATION_MESSAGE, revision: 1, display: { formatMode: "custom", timeZone: { mode: "utc" } } },
      { type: UPDATE_PRESENTATION_MESSAGE, revision: 1, display: { formatMode: "custom", pattern: "YYYY-MM-dd", timeZone: { mode: "utc" } } },
      { type: UPDATE_PRESENTATION_MESSAGE, revision: 1, display: { formatMode: "system", pattern: "yyyy-MM-dd", timeZone: { mode: "utc" } } },
      { type: UPDATE_PRESENTATION_MESSAGE, revision: 1, display: { formatMode: "system", timeZone: { mode: "iana", identifier: "../UTC" } } },
      { type: UPDATE_PRESENTATION_MESSAGE, revision: 1, display: { formatMode: "system", timeZone: { mode: "iana", identifier: "America//New_York" } } },
      Object.assign(Object.create({ revision: 1 }), { type: UPDATE_PRESENTATION_MESSAGE, display })
    ]) {
    ...
    expect(isPresentationUpdateMessage({ type: UPDATE_PRESENTATION_MESSAGE, revision: 1, display: inheritedDisplay })).toBe(false);
  });

  it("requires an exact two-field acknowledgement and optional exact revision", () => {
    expect(isPresentationUpdateAcknowledgement({ type: PRESENTATION_UPDATED_MESSAGE, revision: 4 })).toBe(true);
    expect(isPresentationUpdateAcknowledgement({ type: PRESENTATION_UPDATED_MESSAGE, revision: 4 }, 4)).toBe(true);
    expect(isPresentationUpdateAcknowledgement({ type: PRESENTATION_UPDATED_MESSAGE, revision: 3 }, 4)).toBe(false);
    for (const value of [
      undefined,
      null,
