...
const events = (event: Record<string, unknown>): void => { process.stdout.write(`${JSON.stringify(event)}\n`); };
await runBuildCommand({
    workspaceRoot: process.cwd(), mode: "dev", argv: ["chrome", "--watch"],
    compilerFactory: (config: Parameters<typeof rspack>[0]) => { events({ type: "build-pid", pid: process.pid }); if (phase === "compile") hold(); return rspack(config); }, events,
