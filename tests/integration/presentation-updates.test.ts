...
const CUSTOM_UTC: DisplaySettings = {
    formatMode: "custom",
    pattern: "EEEE, d MMMM yyyy HH:mm 'UTC'",
    timeZone: { mode: "utc" },
};
...
function createDocument(label: string): Document {
    ...
    result.body.innerHTML = `<relative-time datetime="${SOURCE_INSTANT}">`
        + `${label} relative</relative-time>`;
    return result;
}
...
function expected(timeZone: string): string {
    return new Intl.DateTimeFormat(["en-US"], {
        ...
        timeStyle: "short",
        timeZone,
    }).format(new Date(SOURCE_INSTANT));
}
...
function integrationFixture(initialDisplay: DisplaySettings = NEW_YORK) {
    ...
    const documents = new Map<number, Document>([
        [11, createDocument("first")],
        [12, createDocument("second")],
    ]);
        ...
        remove: vi.fn(async (keys: string | readonly string[]) => {
            if (
                ...
                if (failNextDiagnosticRemove) {
                    failNextDiagnosticRemove = false;
                    throw new Error("diagnostics removal unavailable");
                }
    ...
    const tabs = {
        query: vi.fn(
            async (query: { readonly active?: boolean; readonly url?: readonly string[] }) => {
                ...
                if (query.url) {
                    return [
                        { id: 11, url: "https://github.com/one" },
                        { id: 12, url: "https://github.com/two" },
                    ];
                }
                return [];
            },
        ...
        executeScript: vi.fn(
            async ({
                ...
                const addListener =
                    ...
                    vi.fn<(listener: Listener) => void>((listener) => {
                        listenerByTab.set(target.tabId, listener);
                    });
                additions.set(target.tabId, addListener);
                const load =
                    hydration.get(target.tabId) ??
                    vi.fn<() => Promise<unknown>>(() => currentApplication.getDisplayState());
                ...
                installContentRuntime({
                    ...
                    reportDiagnostic: (event) => {
                        ...
                        return currentApplication.recordDocumentEvent(event, {
                            url: "https://github.com/example/repository/issues/"
                                + `${String(target.tabId)}?private=secret#fragment`,
                            frameId: 0,
    ...
    const application = new BackgroundApplication({
    ...
    function restart(): BackgroundApplication {
        currentApplication = new BackgroundApplication({
            settings: new SettingsService(storage),
            coordinator,
            tabs,
            adapters: [adapter],
    ...
    async function settle(): Promise<void> {
        const reads = [...hydration.values()].flatMap((load) =>
...
describe("presentation updates across real background and two documents", () => {
    it("hydrates and updates two real documents with committed custom output", async () => {
        ...
        const sameZone = await fixture.app.setDisplaySettings({
            formatMode: "custom",
            pattern: "yyyy/MM/dd HH:mm XXX",
            timeZone: { mode: "iana", identifier: "America/New_York" },
        });
        ...
        const updateMessages = fixture.tabs.sendMessage.mock.calls.filter(
            ([, message]) =>
        ...
        expect(
            updateMessages
                .slice(0, 2)
                .every(([, message]) => (message as { revision: number }).revision === 4),
        ).toBe(true);
        expect(
            updateMessages
                .slice(2)
                .every(([, message]) => (message as { revision: number }).revision === 5),
        ).toBe(true);
        ...
        expect(
            fixture.documents.get(11)?.querySelectorAll("time[data-no-more-ago-output]"),
        ).toHaveLength(2);
    });

    it("hydrates both documents from IANA settings with exact revisions", async () => {
        ...
        const acknowledgements = await Promise.all(
            fixture.tabs.sendMessage.mock.calls.flatMap(([, message], index) => {
                if (
                    typeof message !== "object" ||
                    message === null ||
                    !("type" in message) ||
    ...
    it.each(["global", "site"] as const)(
        "rehydrates the same %s-disabled documents after a saved zone change",
        async (policy) => {
            const fixture = integrationFixture();
            await fixture.app.ensureReady();
            await fixture.settle();
            ...
            expect(fixture.documents.get(11)?.querySelector("relative-time")?.textContent).toBe(
                "first relative",
            );
    ...
    it.each(["global", "site"] as const)(
        "saves custom output while %s-disabled and hydrates it once on re-enable",
        async (policy) => {
            const fixture = integrationFixture(CUSTOM_NEW_YORK);
            await fixture.app.ensureReady();
            await fixture.settle();
    ...
    it("restores previous policy and document output after corruption", async () => {
        const fixture = integrationFixture(CUSTOM_NEW_YORK);
        await fixture.app.ensureReady();
        await fixture.settle();
        expect(fixture.output(11)?.textContent).toBe("2026-08-23 06:15 -04:00");
    ...
    it.each(["global", "site"] as const)(
        "recovers a saved %s-disabled policy without reactivating owned documents",
        async (policy) => {
            const fixture = integrationFixture();
            await fixture.app.ensureReady();
            await fixture.settle();
            ...
            expect(fixture.stored.sitePreferences).toEqual(
    ...
    it("keeps a future schema untouched when the previous snapshot is valid", async () => {
        const fixture = integrationFixture();
        await fixture.app.ensureReady();
        await fixture.settle();
    ...
    it("resets dual corruption once and reactivates both existing documents", async () => {
        ...
        await expect(restarted.resetAllSettings()).resolves.toMatchObject({
            ok: true,
            acceptedRevision: 0,
    ...
    it("toggles sanitized diagnostics in both existing GitHub documents", async () => {
        const fixture = integrationFixture();
        await fixture.app.ensureReady();
        await fixture.settle();
        expect(fixture.diagnostics).toBeUndefined();
        expect(await fixture.app.getDisplayState()).toMatchObject({
            availability: "ready",
            debugEnabled: false,
        });
        ...
        const enableCalls = fixture.tabs.sendMessage.mock.calls.filter(
            ([, message]) =>
                typeof message === "object" &&
                message !== null &&
                "type" in message &&
        ...
        expect(
            envelope.entries.some((event) => event.incognito && event.pageCategory === "issue"),
        ).toBe(true);
        expect(envelope.entries.every((event) => event.hostname === "github.com")).toBe(true);
        expect(
            envelope.entries.every(
                (event) => event.extensionVersion === "0.1.0" && event.browserFamily === "chromium",
            ),
        ).toBe(true);
        ...
        const diagnosticWrites = fixture.storage.set.mock.calls.filter(([items]) =>
            Object.hasOwn(items, DIAGNOSTICS_STORAGE_KEY),
        ).length;
        const candidate = fixture.documents.get(11)?.createElement("relative-time");
        candidate?.setAttribute("datetime", SOURCE_INSTANT);
        if (candidate) {
    ...
    it("exports both documents safely and clears without stopping future events", async () => {
        const fixture = integrationFixture(CUSTOM_NEW_YORK);
        await fixture.app.ensureReady();
        await fixture.settle();
        ...
        expect(
            result.snapshot.entries.some(
                (entry) =>
                    ...
                    !entry.incognito &&
                    entry.extensionVersion === "0.1.0" &&
                    entry.browserFamily === "chromium",
            ),
        ).toBe(true);
        expect(
            result.snapshot.entries.some(
        ...
        expect(JSON.stringify(result)).not.toMatch(
            /secret page text|private=secret|fragment|2026-08-23T10:15:00/u,
        );
        ...
        expect(afterClear.snapshot.entries.some((entry) => entry.category === "mutation")).toBe(
            true,
        );
        for (const [tabId, page] of fixture.documents) {
    ...
    it("resets opted-in documents without stale diagnostic callbacks", async () => {
        ...
        expect(fixture.stored.display).toEqual({
            formatMode: "system",
        ...
        expect(fixture.additions.get(11)).toHaveBeenCalledTimes(1);
        expect(fixture.additions.get(12)).toHaveBeenCalledTimes(1);
        const resetTeardowns = fixture.tabs.sendMessage.mock.calls.filter(
            ([, message]) =>
                typeof message === "object" &&
                message !== null &&
                "type" in message &&
        ...
        const writesAfterReset = fixture.storage.set.mock.calls.filter(([items]) =>
            Object.hasOwn(items, DIAGNOSTICS_STORAGE_KEY),
        ).length;
        for (const page of fixture.documents.values()) {
            const candidate = page.createElement("relative-time");
    ...
    it.each(["global", "site"] as const)(
        "reactivates two %s-disabled documents and clears every retained preference in one reset",
        async (policy) => {
            const fixture = integrationFixture(CUSTOM_NEW_YORK);
            await fixture.app.ensureReady();
            await fixture.settle();
            ...
            expect(fixture.output(12)).toBeNull();
            const reportsBeforeReset = fixture.diagnosticReports.mock.calls.length;
            const settingsWritesBeforeReset = fixture.storage.set.mock.calls.filter(([items]) =>
            ...
            expect(fixture.diagnostics).toBeUndefined();
            expect(
                fixture.scripting.executeScript.mock.calls
                    .map(([request]) => request.target.tabId)
                    .sort(),
            ).toEqual([11, 12]);
            ...
            for (const page of fixture.documents.values()) {
                const outputs = [...page.querySelectorAll("time[data-no-more-ago-output]")];
                expect(outputs.map((node) => node.textContent)).toEqual([
                    systemOutput,
                    formatter.format(new Date("2026-08-24T11:16:00Z")),
                ]);
            }
            expect(fixture.diagnosticReports).toHaveBeenCalledTimes(reportsBeforeReset);
            expect(fixture.diagnostics).toBeUndefined();
            await expect(fixture.app.getDisplayState()).resolves.toEqual({
                availability: "ready",
                revision: 0,
                display: DEFAULT_SETTINGS_SNAPSHOT.display,
                debugEnabled: false,
            });
    ...
    it("keeps custom documents, atomic settings, and logs after rejected reset", async () => {
        const fixture = integrationFixture(CUSTOM_NEW_YORK);
        await fixture.app.ensureReady();
        await fixture.settle();
        ...
        await expect(fixture.app.resetAllSettings()).resolves.toMatchObject({
            ok: false,
            error: "save-failed",
            state: { availability: "ready", revision: beforeCurrent.revision },
        });
        expect(fixture.stored).toBe(beforeCurrent);
        expect(fixture.storedPrevious).toBe(beforePrevious);
    ...
    it("removes stale diagnostics when corrupt settings recover documents", async () => {
        const fixture = integrationFixture(CUSTOM_NEW_YORK);
        await fixture.app.ensureReady();
        await fixture.app.setDebugEnabled(true);
        await fixture.app.recordDocumentEvent(
