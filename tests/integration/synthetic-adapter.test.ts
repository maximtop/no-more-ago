...
function createConnectedFixture() {
    ...
    githubPage.body.innerHTML = '<relative-time datetime="2026-08-23T10:15:00Z">GitHub relative text</relative-time>';
    const pages = new Map<number, Document>([[11, githubPage], [21, syntheticPage]]);
    const browserTabs = [
    ...
    const scripting = {
        getRegisteredContentScripts: vi.fn(async ({ ids }: { readonly ids: readonly string[] }) => ids.flatMap((id) => {
        ...
        executeScript: vi.fn(async ({ target }: { readonly target: { readonly tabId: number; readonly allFrames: false } }) => {
            ...
            installContentRuntime({
                ...
                reportDiagnostic: (event) => {
                    ...
                    return app.recordDocumentEvent(event, { url: browserTab.url, frameId: 0, tab: { incognito: target.tabId === 21 } });
                },
    ...
    const coordinator = new AdapterActivationCoordinator({ adapters: definitions, scripting, tabs });
...
describe("synthetic adapter through shared document processing", () => {
    it.each(["en-US", "de-DE"])("uses the existing localized System formatter and semantic ownership for %s", (locale) => {
        const page = syntheticDocument();
        const result = processDocument({ url: new URL(SYNTHETIC_URL), root: page, locales: [locale], registry: createSyntheticRegistry() });
        ...
        expect(result[0]?.textContent).toBe(new Intl.DateTimeFormat([locale], { dateStyle: "medium", timeStyle: "short" }).format(new Date(PRIMARY_INSTANT)));
    ...
    it("uses the shared custom UTC presentation without parsing undeclared metadata", () => {
        const page = syntheticDocument();
        const result = processDocument({ url: new URL(SYNTHETIC_URL), root: page, locales: ["en-US"], display: CUSTOM_UTC, registry: createSyntheticRegistry() });
        expect(result.map((output) => output.textContent)).toEqual(["2026-08-25 10:15 UTC", "2026-08-24 08:15 UTC"]);
        expect(page.querySelector("[data-datetime]")?.textContent).toMatch(/metadata only|relative text only/u);
        expect(page.querySelectorAll('time[data-no-more-ago-output][datetime="2026-08-25T10:15:00Z"]')).toHaveLength(1);
    });

    it("reuses one incremental observer for added roots, changed datetime, invalidation, removal, and restoration", async () => {
        const page = syntheticDocument();
        ...
        try {
            ...
            region.innerHTML = '<time-ago class="synthetic-event" datetime="2026-08-26T11:16:00Z">new relative activity</time-ago>';
...
describe("synthetic adapter through real settings, activation, diagnostics, and content", () => {
    it("registers both injected adapters, activates real documents, and exposes the synthetic site's public policy", async () => {
        const fixture = createConnectedFixture();
        try {
            await fixture.app.ensureReady("startup");
            await fixture.settle();
            expect([...fixture.registrations.keys()].sort()).toEqual(["no-more-ago-github", "no-more-ago-synthetic"]);
            expect(fixture.registrations.get("no-more-ago-synthetic")?.matches).toEqual(["http://synthetic.test/*", "https://synthetic.test/*"]);
            ...
            await expect(fixture.app.getPopupState()).resolves.toMatchObject({ availability: "ready", hostname: SYNTHETIC_HOSTNAME, hasAdapter: true, siteEnabled: true, status: "active" });
    ...
    it("applies committed custom/UTC settings to existing and future synthetic sources", async () => {
        ...
        try {
            await fixture.app.ensureReady("startup");
            await fixture.settle();
            await expect(fixture.app.setDisplaySettings(CUSTOM_UTC)).resolves.toMatchObject({ ok: true, acceptedRevision: 1, refreshFailures: [] });
            expect(outputs(fixture.syntheticPage).map((output) => output.textContent)).toEqual(["2026-08-25 10:15 UTC", "2026-08-24 08:15 UTC"]);
    ...
    it("collects sanitized synthetic diagnostics only while opted in and isolates journal failures", async () => {
        ...
        try {
            ...
            await expect(fixture.app.setDebugEnabled(true)).resolves.toMatchObject({ ok: true, acceptedRevision: 1, state: { enabled: true } });
            ...
            expect(JSON.stringify(fixture.diagnostics)).not.toMatch(/private=secret|fragment|2026-08-27|sensitive synthetic document content|synthetic\.test\/activity/u);
            ...
            expect(fixture.additions.get(21)).toHaveBeenCalledTimes(1);
            await expect(fixture.app.setDebugEnabled(false)).resolves.toMatchObject({ ok: true, state: { enabled: false } });
            expect(fixture.diagnostics).toBeUndefined();
