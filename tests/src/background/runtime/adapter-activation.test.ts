...
function fakes() {
    ...
    const scripting = {
        ...
        executeScript: vi.fn(
            async (_input: { target: { tabId: number; allFrames: false }; files: string[] }) =>
                undefined,
        ),
    };
    const tabs = {
        ...
        sendMessage: vi.fn(async (tabId: number, message: unknown) =>
            ...
            message.type === DOCUMENT_STATUS_MESSAGE
                ? { type: DOCUMENT_STATUS_MESSAGE, phase: phases.get(tabId) ?? "active" }
                : undefined,
        ),
    };
...
describe("AdapterActivationCoordinator", () => {
    ...
    it("does not reinject a cold worker when its registration is present", async () => {
        const fake = fakes();
    ...
    it("updates a mismatched registration and sweeps an activation event", async () => {
        const fake = fakes();
        fake.registered.set(github.registration.id, {
            ...github.registration,
            js: ["old-content.js"],
        });
        const coordinator = new AdapterActivationCoordinator({ adapters: [github], ...fake });
        ...
        const sweep = await coordinator.reconcile({
            revision: 4,
    ...
    it("injects on unchanged settings and skips a stopped disabled runtime", async () => {
        const fake = fakes();
    ...
    it("reselects adapters from URLs and isolates sibling tab failures", async () => {
        const fake = fakes();
        ...
        fake.scripting.executeScript.mockImplementationOnce(
            async (input: { target: { tabId: number; allFrames: false }; files: string[] }) => {
        ...
        expect(fake.scripting.executeScript).toHaveBeenCalledWith({
            target: { tabId: 4, allFrames: false },
            files: ["content.js"],
        });
        ...
        const disabled = await coordinator.reconcile({
            revision: 8,
    ...
    it.each(["get", "register", "update", "unregister"] as const)(
        "attributes %s registration failure",
        async (operation) => {
            const fake = fakes();
            if (operation === "get") {
                fake.scripting.getRegisteredContentScripts.mockRejectedValueOnce(
                    new Error(operation),
                );
            }
            ...
            const result = await coordinator.reconcile({
                revision: 9,
                mode: operation === "unregister" ? "settings-change" : "cold-worker",
                policy: operation === "unregister" ? "disabled" : "enabled",
            });
            expect(result.failures).toContainEqual({
                scope: "registration",
                adapterId: "synthetic",
    ...
    it("attributes query, status, teardown, and sibling continuation failures", async () => {
        ...
        const coordinator = new AdapterActivationCoordinator({ adapters: [github], ...fake });
        const queryFailure = await coordinator.reconcile({
            revision: 10,
            mode: "activation-sweep",
            policy: "enabled",
        });
        expect(queryFailure.failures).toContainEqual({
            scope: "matching-tabs-query",
            adapterId: "synthetic",
        });
        ...
        const statusCoordinator = new AdapterActivationCoordinator({
            adapters: [github],
            ...statusFake,
        });
        const statusFailure = await statusCoordinator.reconcile({
            revision: 11,
            mode: "cold-worker",
            policy: "disabled",
        });
        expect(statusFailure.failures).toContainEqual({
            scope: "tab",
            adapterId: "synthetic",
            tabId: 3,
            action: "status",
        ...
        const teardownCoordinator = new AdapterActivationCoordinator({
            adapters: [github],
            ...teardownFake,
        });
        const teardownFailure = await teardownCoordinator.reconcile({
            revision: 12,
            mode: "settings-change",
            policy: "disabled",
        });
        expect(teardownFailure.failures).toContainEqual({
            scope: "tab",
            adapterId: "synthetic",
            tabId: 3,
            action: "teardown",
    ...
    it("reconciles adapters independently across disable and reenable", async () => {
        const fake = multiFakes();
        const coordinator = new AdapterActivationCoordinator({
            adapters: [github, sibling],
            ...fake,
        });
        ...
        const disabled = await coordinator.reconcile({
            ...
            mode: "settings-change",
            policy: "enabled",
            sitePreferences: { "synthetic.test": false },
            affectedHostnames: ["synthetic.test"],
        });
        expect(disabled.failures).toEqual([]);
        ...
        fake.tabs.query.mockClear();
        fake.tabs.sendMessage.mockClear();
        const reenabled = await coordinator.reconcile({
            revision: 3,
            mode: "settings-change",
            policy: "enabled",
            sitePreferences: { "synthetic.test": true },
            affectedHostnames: ["synthetic.test"],
        });
