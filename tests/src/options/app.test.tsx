...
describe('Options Sites contract', () => {
    ...
    it('ignores an actual stale lower-revision Sites response', async () => {
        ...
        const rendered = await renderOptions(ready, {
            transport: {
                sendMessage: (message) => (message.type === SET_SITE_ENABLED_MESSAGE
                    ? Promise.resolve({
                        ok: true,
                        acceptedRevision: 3,
                        surface: SITE_SETTINGS_SURFACE.SITES,
...
describe('Options reset contract', () => {
    ...
    it('keeps recovery unavailable after a typed persistence failure', async () => {
        const rendered = await renderOptions(unavailableSites, {
            transport: {
                sendMessage: (message) => (message.type === RESET_ALL_SETTINGS_MESSAGE
                    ? Promise.resolve({
                        ok: false,
                        error: 'save-failed',
...
describe('Options Display contract', () => {
    ...
    it('does not replace the committed display with a stale lower-revision response', async () => {
        ...
        const rendered = await renderOptions(ready, {
            transport: {
                sendMessage: (message) => (message.type === SET_DISPLAY_SETTINGS_MESSAGE
                    ? Promise.resolve({
                        ok: true,
                        acceptedRevision: 3,
                        state: stale,
        ...
        const rendered = await renderOptions(ready, {
            transport: {
                sendMessage: (message) => {
                    if (message.type === SET_DISPLAY_SETTINGS_MESSAGE) {
                        writes.push(message);
                        const { display } = (message as { display: DisplaySettings });
                        return Promise.resolve({
                            ok: true,
                            acceptedRevision: 5,
...
describe('Options Debug logs contract', () => {
    ...
    it('explains an empty journal when a download is requested', async () => {
        const rendered = await renderOptions(ready, {
            transport: {
                sendMessage: (message) => (message.type === GET_DIAGNOSTICS_SNAPSHOT_MESSAGE
