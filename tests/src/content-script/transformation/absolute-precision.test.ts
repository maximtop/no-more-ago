...
function processLabel(label: string, settings = display, datetime = instant) {
    ...
    return processDocument({
        url: new URL("https://example.test"), root: document,
        locales: ["en-US"], display: settings,
    });
}
...
describe("absolute date policy", () => {
    ...
    it.each(["Aug 22, 2026", "2026-08-22", "2026-08-22 09:19"])(
    ...
    it.each(["", "Unknown", "2026-08-22 09:19:17", "09:19", "Aug 21, 2026"])(
    ...
    it("does not infer a missing zone or invent seconds missing from the source", () => {
        expect(processLabel("Aug 22, 2026", { ...display, precisionPolicy: expanded },
            "2026-08-22T09:19:17")).toHaveLength(0);
        expect(processLabel("2026-08-22 09:19", { ...display, precisionPolicy: expanded },
            "2026-08-22T09:19Z")).toHaveLength(0);
    });
...
describe("age-based output", () => {
    ...
    it("uses one age boundary for identical timestamps in the same document pass", () => {
        document.body.innerHTML = [1, 2].map(() =>
            `<time datetime="${instant}">1 day ago</time>`).join("");
        vi.spyOn(Date, "now")
            .mockReturnValueOnce(now.getTime())
            .mockReturnValue(now.getTime() + 1);
        const outputs = processDocument({
            url: new URL("https://example.test"), root: document,
            locales: ["en-US"], display: { ...display, precisionPolicy: aged },
        });
    ...
    it("uses the first range for future timestamps without enabling absolute labels", () => {
        expect(processLabel("in 1 day", { ...display, precisionPolicy: aged },
            "2026-08-24T09:19:17Z")[0]?.textContent).toBe("2026-08-24 09:19:17");
    ...
    it.each([
        ...
        expect(processLabel("3 days ago", {
            formatMode: FORMAT_MODE.CUSTOM, pattern,
            timeZone: { mode: TIME_ZONE_MODE.IANA, identifier: "Pacific/Honolulu" },
            precisionPolicy: aged,
