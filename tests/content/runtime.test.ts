...
describe("installContentRuntime", () => {
    ...
    it("processes ready documents synchronously and installs idempotently", () => {
    ...
    it("rolls back a failed loading readiness callback and retries a fresh generation", () => {
        ...
        const handle = install(messages);
        const firstCallback = readiness.mock.calls.find(
        ...
        observe.mockRestore();
        const installsBeforeRetry = readiness.mock.calls.filter(
            ([type]) => type === "DOMContentLoaded",
        ).length;
        expect(install(messages)).toBe(handle);
    ...
    it("reports waiting, active, and stopped phases through the singular listener", () => {
        const messages = createMessages();
        const handle = install(messages);
        expect(messages.dispatch({ type: DOCUMENT_STATUS_MESSAGE })).toEqual({
    ...
    it("cleans partial startup before retrying dynamic candidates", async () => {
        setReadyState("complete");
        const messages = createMessages();
    ...
    it("waits for hydration before ownership and uses UTC initially", async () => {
        setReadyState("complete");
        const messages = createMessages();
    ...
    it("shares hydration across activations and waits for readiness", async () => {
        const messages = createMessages();
        let resolveLoad: ((value: unknown) => void) | undefined;
        const load = vi.fn(
            () =>
    ...
    it("accepts newer updates without regressing to older hydration", async () => {
        setReadyState("complete");
        const messages = createMessages();
        let resolveLoad: ((value: unknown) => void) | undefined;
        ...
        expect(
            messages.dispatch({
                ...
                revision: 8,
                display: state(8, "utc").display,
            }),
        ).toEqual({ type: PRESENTATION_UPDATED_MESSAGE, revision: 8 });
    ...
    it("applies newer revisions once and rejects older or stopped updates", async () => {
        setReadyState("complete");
        const messages = createMessages();
        ...
        expect(
            messages.dispatch({
                type: UPDATE_PRESENTATION_MESSAGE,
                revision: 5,
                display: state(5, "utc").display,
            }),
        ).toBeUndefined();
        reformat.mockRestore();
    });

    it("invalidates old hydration before reactivating a new generation", async () => {
        setReadyState("complete");
        const messages = createMessages();
    ...
    it.each(["global", "site"] as const)(
        "rehydrates the same retained document after %s disable and a saved zone change",
        async (policy) => {
            setReadyState("complete");
            const messages = createMessages();
            ...
            expect(firstOutput?.textContent).toBe(
                new Intl.DateTimeFormat(["en-US"], {
                    dateStyle: "medium",
                    timeStyle: "short",
            ...
            const reactivated = installContentRuntime({
                document,
                url: new URL("https://github.com/example/repo"),
                locales: ["en-US"],
                loadDisplayState: load,
    ...
    it("resolves live locales for new candidates without polling", async () => {
        setReadyState("complete");
        const messages = createMessages();
        ...
        expect(outputs[1]?.textContent).toBe(
            new Intl.DateTimeFormat(["en-GB"], { dateStyle: "medium", timeStyle: "short" }).format(
                new Date("2026-08-23T10:15:00Z"),
            ),
        );
    });
    ...
    it("fails closed for unavailable or malformed hydration responses", async () => {
        ...
        for (const response of [
            ...
            installContentRuntime({
                ...
                url: new URL("https://github.com/example/repo"),
                locales: ["en-US"],
                loadDisplayState: () => Promise.resolve(response),
                messages,
            });
            await Promise.resolve();
            await Promise.resolve();
            expect(messages.dispatch({ type: DOCUMENT_STATUS_MESSAGE })).toMatchObject({
                phase: "failed",
            });
            expect(document.querySelector("time[data-no-more-ago-output]")).toBeNull();
            expect(document.querySelector("relative-time")?.hasAttribute("hidden")).toBe(false);
    ...
    it("enables and disables diagnostic processing on the single existing listener", async () => {
        setReadyState("complete");
        const messages = createMessages();
        const report = vi.fn<(event: Record<string, unknown>) => Promise<void>>(() =>
            Promise.resolve(),
        );
        ...
        expect(
            messages.dispatch({ type: UPDATE_DEBUG_POLICY_MESSAGE, revision: 4, enabled: true }),
        ...
        expect(
            messages.dispatch({ type: UPDATE_DEBUG_POLICY_MESSAGE, revision: 3, enabled: false }),
        ).toBeUndefined();

        expect(
            messages.dispatch({ type: UPDATE_DEBUG_POLICY_MESSAGE, revision: 5, enabled: false }),
    ...
    it("hydrates diagnostics without letting rejected reports break dates", async () => {
        setReadyState("complete");
        const messages = createMessages();
        const report = vi.fn<(event: Record<string, unknown>) => Promise<void>>(() =>
    ...
    it("keeps a synthetic hostname unprocessed when the production registry is omitted", () => {
        setReadyState("complete");
        document.body.innerHTML = '<time-ago class="synthetic-event" '
            + 'datetime="2026-08-25T10:15:00Z">three hours ago</time-ago>';
        const messages = createMessages();
        const handle = installContentRuntime({
            document,
            url: new URL("https://synthetic.test/example"),
            locales: ["en-US"],
            messages,
        });
        expect(messages.dispatch({ type: DOCUMENT_STATUS_MESSAGE })).toMatchObject({
    ...
    it("forwards injected registry through the content lifecycle", async () => {
        setReadyState("complete");
        document.body.innerHTML = '<time-ago class="synthetic-event" '
            + 'datetime="2026-08-25T10:15:00Z">private relative wording</time-ago>';
        ...
        try {
            ...
            expect(output?.textContent).toBe(
                new Intl.DateTimeFormat(["en-US"], {
                    ...
                    timeStyle: "short",
                    timeZone: "UTC",
                }).format(new Date("2026-08-25T10:15:00Z")),
            );
            expect(report).not.toHaveBeenCalled();
            ...
            expect(JSON.stringify(report.mock.calls)).not.toMatch(
                /private relative wording|private=secret|2026-08-26/u,
            );
