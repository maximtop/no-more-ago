...
const sender = (url = "https://github.com/acme/project/issues/3", incognito = false) => ({
    url,
    tab: { incognito },
});
...
describe("diagnostic event privacy", () => {
    it("derives only coarse trusted sender context", () => {
        expect(deriveDiagnosticContext(sender())).toEqual({
            hostname: "github.com",
            pageCategory: "issue",
            incognito: false,
        });
        expect(
            deriveDiagnosticContext(
                sender("https://github.com/acme/project?token=secret#dom", true),
            ),
        ).toMatchObject({ hostname: "github.com", pageCategory: "repository", incognito: true });
        expect(deriveDiagnosticContext(sender("http://github.com/acme/project"))).toMatchObject({
            hostname: "github.com",
    ...
    it("allows technical scalars and strips arbitrary stack data", () => {
        const event = createDiagnosticEvent(
            {
                ...
                stack: "Error: secret https://github.com/?token=x\n at fn (/Users/max/app.ts:12:7)",
            },
            sender(),
            123,
        );
        ...
        expect(JSON.stringify(event)).not.toMatch(
            /github\.com\/|token|Users|max|secret|datetime|<body>/u,
        );
    });
    ...
    it.each(DIAGNOSTIC_BROWSER_FAMILIES)(
        ...
        (browserFamily) => {
            const event = createDiagnosticEvent(
                { category: "lifecycle", browserFamily },
                sender(),
                4,
            );
    ...
    it("drops a spoofed browser family and rejects content-owned private context", () => {
        const event = createDiagnosticEvent(
            { category: "lifecycle", browserFamily: "safari", incognito: true },
            sender("https://github.com/acme/project", false),
            4,
        );
        expect(event).toBeNull();
        expect(
            createDiagnosticEvent(
                { category: "lifecycle", browserFamily: "safari" },
                sender("https://github.com/acme/project", false),
                4,
            ),
        ).not.toMatchObject({ browserFamily: "safari" });
    });

    it("rejects substituted context, forbidden fields, and inherited input", () => {
        expect(
            sanitizeDiagnosticEvent(
                { category: "mutation", datetime: "2026-08-25", text: "page" },
                { hostname: "github.com", pageCategory: "repository", incognito: false },
                1,
            ),
        ).toBeNull();
        expect(
            sanitizeDiagnosticEvent(
                { category: "mutation" },
                { hostname: "https://evil.example", pageCategory: "repository", incognito: false },
                1,
            ),
        ).toBeNull();
        const inherited = Object.assign(
            Object.create({ reason: "processing-failed" }) as Record<string, unknown>,
            { category: "error" },
        );
        expect(
            sanitizeDiagnosticEvent(
                inherited,
                { hostname: "github.com", pageCategory: "other", incognito: false },
                1,
            ),
        ).toBeNull();
    });
...
describe("diagnostic runtime envelopes", () => {
    ...
    it("accepts only an exact diagnostic event envelope", () => {
        ...
        expect(
            isDiagnosticEventMessage({
                type: DIAGNOSTIC_EVENT_MESSAGE,
                event: { category: "mutation", count: 1, datetime: "secret" },
            }),
        ).toBe(false);
        expect(
            isDiagnosticEventMessage(
                Object.assign(
                    Object.create({ type: DIAGNOSTIC_EVENT_MESSAGE }) as Record<string, unknown>,
        ...
        expect(
            isDiagnosticEventMessage({
                type: DIAGNOSTIC_EVENT_MESSAGE,
                event: Object.assign(Object.create({ count: 1 }) as Record<string, unknown>, {
                    category: "mutation",
                }),
            }),
        ).toBe(false);
