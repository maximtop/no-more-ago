...
Exchange Q&A sites, Telegram Web K, and TikTok, while keeping extraction
separate from shared timestamp validation and rendering. Instagram uses a
...
presentation. Public `https://t.me/s/*` pages use the generic source.
...
- **Current site support:** Generic HTTP(S) `time[datetime]` processing is
  ...
  contains GitHub, Hacker News, Stack Exchange, Telegram Web K, and TikTok as
...
- Keep GitHub-, Hacker News-, Stack Exchange-, Instagram-, Telegram Web K-, and
  TikTok-specific selectors, timestamp sources, presentation rules, and trusted
  evidence logic inside their respective adapters so adding or changing one
  site does not add branches to shared transformation code. Keep TikTok URL,
  selector, hydration, and ID-decoding knowledge in
