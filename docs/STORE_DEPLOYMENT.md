...
The extension reaches Chrome, Edge, and Firefox through one release-to-store
boundary. Store submission is deliberately separate from release creation, so
creating a GitHub Release never changes a store listing by itself.
...
The deployment consists of these files:
...
reviewer-notes path and summary. Build and package inputs stay in `.github/workflows/ci.yml`; the release
title stays in `.github/workflows/release.yml`.
