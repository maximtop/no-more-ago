...
Development includes source maps; release omits them. Safari is unsupported.
...
The Makefile wraps pnpm builds; `make start [browser]` enables watch mode.
...
Tagged releases publish a GitHub Release with the store archives and
checksums; store submission is a separate manual workflow. See
[docs/RELEASE.md](docs/RELEASE.md) for the release process, the Chrome Web
Store deployment, store configuration, local store commands, and the failure
playbook.
