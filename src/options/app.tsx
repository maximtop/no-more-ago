...
export function OptionsApp({
    ...
    const [displayDraft, setDisplayDraft] = useState<DisplayDraft | undefined>(
        initialDisplayState?.availability === "ready"
            ? draftFromDisplay(initialDisplayState.display)
            : undefined,
    );
    ...
    const [resetOrigin, setResetOrigin] = useState<ResetOrigin>(
        initialState?.availability ?? "unavailable",
    );
    ...
    const onResetAllSettings = async (): Promise<void> => {
        ...
        if (result.kind === "response") {
            if (result.response.ok) {
                ...
                setResetNotice(undefined);
                setDisplayDraft(
                    draftFromDisplay({ formatMode: "system", timeZone: { mode: "system" } }),
                );
    ...
    const onDisplaySave = async (): Promise<void> => {
        if (
            !displayState ||
            displayState.availability !== "ready" ||
            !displayDraft ||
            savingDisplay
        ) {
            return;
        ...
        if (result.kind === "response") {
            ...
            if (
                responseState.availability !== "ready" ||
                responseState.revision >= displayState.revision
            ) {
            ...
            if (
                result.state.availability !== "ready" ||
                result.state.revision >= displayState.revision
            ) {
    ...
    const onDebugChange = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
        ...
        if (result.kind === "response") {
            if (
                ...
                result.response.state.revision >= debugState.revision
            ) {
            ...
            if (
                result.state.availability !== "ready" ||
                result.state.revision >= debugState.revision
            ) {
    ...
    const diagnosticsErrorText = (
        error: "disabled" | "unavailable" | "empty" | "invalid-journal" | "storage-failed",
    ): string => {
        if (error === "disabled") {
    ...
    const patternError =
        displayDraft?.formatMode === "custom"
            ? customPatternError(displayDraft.pattern)
            : undefined;
    const preview =
        displayDraft?.formatMode === "custom" && !patternError
            ? formatDateWithPresentation(
                PREVIEW_INSTANT,
                previewLocales(),
                displayFromDraft(displayDraft),
            )
            : undefined;
    return (
        <MantineProvider>
                    ...
                    <Stack gap="md">
                        ...
                        {state.availability === "unavailable" ? (
                            ...
                            <>
                                {!state.globalEnabled ? (
                                    <Alert color="blue">
                                        The extension is off. Site preferences are saved and will
                                        apply when the extension is enabled.
                                    </Alert>
                                ) : null}
                                <Stack component="section" aria-labelledby="sites-heading" gap="sm">
                                    ...
                                    {state.sites.length === 0 ? (
                                        <Text role="status">
                                            No site preferences have been saved.
                                        </Text>
                                    ) : null}
                                </Stack>
                            </>
                        ...
                        <Box component="section" aria-labelledby="display-heading">
                            ...
                            {displayLoading ? (
                                <Text role="status">Loading display settings…</Text>
                            ) : null}
                            {!displayLoading && displayState?.availability === "unavailable" ? (
                                <Text role="status">{unavailableDisplayText(displayState)}</Text>
                            ) : null}
                            {!displayLoading &&
                            displayState?.availability === "ready" &&
                            displayDraft ? (
                                            ...
                                            <TextInput
                                                ...
                                                error={
                                                    displayNotice === "invalid-time-zone"
                                                        ? displayNoticeText(displayNotice)
                                                        : undefined
                                                }
                                                disabled={savingDisplay}
                                            />
                        ...
                        {state.availability === "ready" ? (
                            <Box component="section" aria-labelledby="debug-heading">
                                ...
                                {debugLoading ? (
                                    <Text role="status">Loading debug settings…</Text>
                                ) : null}
                                {!debugLoading && debugState?.availability === "unavailable" ? (
                                    <Text role="status">
                                    ...
                                    <Stack gap="xs" mt="xs">
                                        ...
                                        <Stack gap="xs" style={{ flexDirection: "row" }}>
                                            <Button
                                                ...
                                                disabled={!debugState.enabled || diagnosticsBusy}
                                            >
                                                Download logs
                                            </Button>
                                            <Button
                                        ...
                                        {diagnosticsNotice ? (
                                            <Alert
                                                role="status"
                                                color={
                                                    diagnosticsNotice === "Diagnostic logs cleared."
                                                        ? "green"
                                                        : "red"
                                                }
                                            >
                        ...
                        {resetNotice ? (
                            <Alert role="alert" color="red">
                                {resetNoticeText(resetNotice, resetOrigin)}
                            </Alert>
                        ) : null}
                    </Stack>
                </Paper>
