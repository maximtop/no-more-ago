...
function installApplication(): BackgroundApplication | undefined {
    const candidate = chrome as unknown as {
        ...
        readonly tabs?: { readonly query?: TabsRuntime["query"]; readonly sendMessage?: TabsRuntime["sendMessage"] };
        readonly scripting?: {
            readonly getRegisteredContentScripts?: typeof chrome.scripting.getRegisteredContentScripts;
    ...
    const scripting: ScriptingRuntime = {
        getRegisteredContentScripts: (filter) => candidate.scripting?.getRegisteredContentScripts?.(filter as { ids: string[] }).then((scripts) => scripts.map((script) => ({
            id: script.id,
        ...
        registerContentScripts: (scripts) => candidate.scripting?.registerContentScripts?.(scripts) ?? Promise.reject(new Error("Scripting is unavailable")),
        updateContentScripts: (scripts) => candidate.scripting?.updateContentScripts?.(scripts) ?? Promise.reject(new Error("Scripting is unavailable")),
        unregisterContentScripts: (filter) => candidate.scripting?.unregisterContentScripts?.(filter) ?? Promise.reject(new Error("Scripting is unavailable")),
        executeScript: (input) => candidate.scripting?.executeScript?.(input as unknown as Parameters<typeof chrome.scripting.executeScript>[0]) ?? Promise.reject(new Error("Scripting is unavailable"))
    };
    const tabs: TabsRuntime = {
        query: async (query) => {
            const result = await candidate.tabs?.query?.(query);
            return (result ?? []).flatMap((tab) => tab.id === undefined ? [] : [typeof tab.url === "string" ? { id: tab.id, url: tab.url } : { id: tab.id }]);
        },
        sendMessage: (tabId, message, options) => candidate.tabs?.sendMessage?.(tabId, message, options) as Promise<unknown>
    };
    const coordinator = new AdapterActivationCoordinator({ adapters: [githubRuntimeDefinition], scripting, tabs });
    ...
    return new BackgroundApplication({ settings: new SettingsService(storage), coordinator, tabs, adapters: [githubRuntimeDefinition], journal: new DiagnosticJournal(storage), diagnosticEnvironment });
}
...
if (application && chrome.runtime?.onMessage?.addListener) {
    chrome.runtime.onMessage.addListener((message: unknown, sender, sendResponse) => {
        ...
        if (isGetPopupStateMessage(message)) {
            void application.getPopupState().then(sendOnce, () => sendOnce({ availability: "unavailable", revision: null, globalEnabled: null, hostname: null, siteEnabled: null, hasAdapter: false, status: "settings-unavailable", failure: "settings-load" }));
            return true;
        }
        if (isGetDisplayStateMessage(message)) {
            void application.getDisplayState().then(sendOnce, () => sendOnce({ availability: "unavailable", revision: null, display: null, failure: "settings-load" }));
            return true;
        }
        if (isGetDebugStateMessage(message)) {
            void application.getDebugState().then(sendOnce, () => sendOnce({ availability: "unavailable", revision: null, enabled: null, failure: "settings-load" }));
            return true;
        }
        if (isSetDebugEnabledMessage(message)) {
            void application.setDebugEnabled(message.enabled).then(sendOnce, () => sendOnce({ ok: false, error: "settings-unavailable", state: { availability: "unavailable", revision: null, enabled: null, failure: "settings-load" } }));
            return true;
        }
        if (isDiagnosticEventMessage(message)) {
            void application.recordDocumentEvent(message.event, sender).then((accepted) => sendOnce({ ok: accepted }), () => sendOnce({ ok: false }));
            return true;
        }
        if (isSetDisplaySettingsMessage(message)) {
            void application.setDisplaySettings(message.display).then(sendOnce, () => sendOnce({ ok: false, error: "settings-unavailable", state: { availability: "unavailable", revision: null, display: null, failure: "settings-load" } }));
            return true;
        }
        if (isSetGlobalEnabledMessage(message)) {
            void application.setGlobalEnabled(message.enabled).then(sendOnce, () => sendOnce({ ok: false, error: "settings-unavailable", state: { availability: "unavailable", revision: null, globalEnabled: null, hostname: null, siteEnabled: null, hasAdapter: false, status: "settings-unavailable", failure: "settings-load" } }));
            return true;
        }
        if (isGetSitesStateMessage(message)) {
            void application.getSitesState().then(sendOnce, () => sendOnce({ availability: "unavailable", revision: null, globalEnabled: null, sites: [], failure: "settings-load" }));
            return true;
        }
        if (isResetAllSettingsMessage(message)) {
            void application.resetAllSettings().then(sendOnce, () => sendOnce({ ok: false, error: "settings-unavailable", state: { availability: "unavailable", revision: null, globalEnabled: null, sites: [], failure: "settings-load" } }));
            return true;
        }
        if (isSetSiteEnabledMessage(message)) {
            void application.setSiteEnabled(message.hostname, message.enabled, message.surface).then(sendOnce, () => sendOnce({
                ok: false,
                error: "settings-unavailable",
                surface: message.surface,
                state: message.surface === "popup"
                    ? { availability: "unavailable", revision: null, globalEnabled: null, hostname: null, siteEnabled: null, hasAdapter: false, status: "settings-unavailable", failure: "settings-load" }
                    : { availability: "unavailable", revision: null, globalEnabled: null, sites: [], failure: "settings-load" }
            }));
            return true;
        }
        return false;
